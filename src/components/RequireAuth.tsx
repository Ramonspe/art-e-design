...
export const RequireDeveloper = ({ children }: { children: JSX.Element }) => {
  const { user, isAdmin, isDeveloper, loading } = useAuth();
  if (loading) return <div className="container py-20 text-center text-muted-foreground">Carregando…</div>;
  if (!user) return <Navigate to="/auth?redirect=/desenvolvedor" replace />;
  if (!isAdmin && !isDeveloper) return <div className="container py-20 text-center"><h1 className="text-2xl font-bold">Acesso restrito</h1><p className="mt-2 text-muted-foreground">Esta área é exclusiva para desenvolvedores autorizados.</p></div>;
  return children;
};
