...
const Account = () => {
  ...
  const saveProfile = async (event: React.FormEvent) => {
    ...
    const addressComplete = [addressPayload.cep, addressPayload.street, addressPayload.number, addressPayload.district, addressPayload.city, addressPayload.state].every(Boolean);
  ...
  const changeEmail = async (event: React.FormEvent) => {
    ...
    if (error) { toast.error(error.message); return; }
    setNewEmail(""); toast.success("E-mail atualizado com sucesso.");
  };

  return <div className="container py-10 max-w-5xl">
    <div className="flex items-start justify-between mb-8 gap-4 flex-wrap"><div><h1 className="text-3xl font-bold">Minha conta</h1><p className="text-muted-foreground mt-1">{user?.email}</p></div><div className="flex gap-2">{isAdmin && <Button asChild variant="gold"><Link to="/admin">Painel admin</Link></Button>}<Button asChild variant="outline"><Link to="/meus-pedidos">Meus pedidos</Link></Button><Button variant="outline" onClick={signOut}><LogOut className="h-4 w-4" /> Sair</Button></div></div>
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6"><form onSubmit={changePassword} className="rounded-xl border border-border bg-card p-6 space-y-3"><h2 className="flex items-center gap-2 text-xl font-bold"><KeyRound className="h-5 w-5 text-primary" /> Trocar senha</h2><Input label="Nova senha" type="password" value={password} onChange={setPassword} minLength={6} required /><Input label="Confirmar nova senha" type="password" value={passwordConfirmation} onChange={setPasswordConfirmation} minLength={6} required /><Button type="submit" variant="outline">Atualizar senha</Button></form>
        <form onSubmit={changeEmail} className="rounded-xl border border-border bg-card p-6 space-y-3"><h2 className="text-xl font-bold">Trocar e-mail</h2><p className="text-sm text-muted-foreground">Informe o novo endereço que deseja usar na conta.</p><Input label="Novo e-mail" type="email" value={newEmail} onChange={setNewEmail} required /><Button type="submit" variant="outline">Atualizar e-mail</Button></form>
      </div>
    </div>
  </div>;
};
