    ...
    <div className="space-y-6">
      <div>
        ...
        <ul className="space-y-1">
          <li>
            <button onClick={() => setCat("all")} className={`w-full text-left px-3 py-2 rounded-md text-sm transition-smooth ${cat === "all" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>Todas as categorias</button>
          </li>
          {categories.map((c) => (
            <li key={c.slug}>
              <button onClick={() => setCat(c.slug)} className={`w-full text-left px-3 py-2 rounded-md text-sm transition-smooth ${cat === c.slug ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>
                {c.name}
              </button>
            </li>
          ))}
        ...
        <div>
          <div className="flex items-center justify-between mb-5 gap-3">
            ...
            <select
              ...
              className="ml-auto h-10 rounded-md border border-input bg-background px-3 text-sm"
              aria-label="Ordenar"
            >
          ...
          {filtered.length === 0 ? (
            <p className="py-20 text-center text-muted-foreground">Nenhum produto encontrado nessa faixa.</p>
          ) : (
